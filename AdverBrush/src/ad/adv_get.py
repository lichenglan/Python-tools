...
# 获取百度视频中的广告（暂停时刷新广告）
def getBiduVideoAd(baiduCount):
  url = baiduUrlHeader + str(getCurrentTime())
  for x in range(100):
    try:
      res = requests.get(url, verify=False)
      print('《百度视频》第' + str(x) + '次获取广告数据 ===========>')
      print('状态码==>', res.status_code)
      print('报文头==>', res.headers)
      print('body ==>', res.text)
      resp = json.loads(res.text)

      baiduCount = baiduCount + 1
      print("baiduCount ========>", baiduCount)
...
# 获取行家说说的广告
def getHangjiashuoAd(hangjiashuoCount):
  url = 'http://s.lmstyle.cn/ad/get'
  for x in range(100):
    try:
      res = requests.post(url, json.dumps(hangjiashuoBody), hangjiashuoHeaders)
      print('《行家说说》第' + str(x) + '次获取广告数据 ===========>')
      print('状态码==>', res.status_code)
      print('body ==>', res.text)
      resp = json.loads(res.text)

      hangjiashuoCount = hangjiashuoCount + 1
      print("hangjiashuoCount ========>", hangjiashuoCount)
      ...
      # 重新请求
      time.sleep(100)
      getHangjiashuoAd(hangjiashuoCount)
    except AttributeError as e:
      print("Tag was not found")


# 获取驾校一点通广告
def getJiaxiaoAd(jiaxiaoCount):
  url = jiaxiaoHeader + str(currentTime) + jiaxiaoEnd
  for x in range(100):
    try:
      res = requests.get(url, verify=False)
      print('《驾校一点通广告》第' + str(x) + '次获取广告信息====>' + url)
      print('状态码==>', res.status_code)
      print('报文头==>', res.headers)
      print('body ==>', res.text)
      resp = json.loads(res.text)

      jiaxiaoCount = jiaxiaoHeader + 1
      print("jiaxiaoCount ========>", jiaxiaoCount)
